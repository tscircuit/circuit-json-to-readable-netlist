...
it("keeps MIPI CSI and DSI lane hints for generic chip pin labels", () => {
  const circuitJson = [
    ...
    },
    {
      ...
      connected_source_port_ids: ["source_port_20", "source_port_21"],
      connected_source_net_ids: [],
    },
    {
      type: "source_trace",
  ...
  expect(
    ...
    NET: U1_MIPI_DPHY_D3_N
      - U1 pin24 (MIPI_DPHY_D3_N)
      - J1 pin11 (MIPI_DPHY_D3_N)
...
it("scores MIPI lane aliases without broadening generic numbered pins", () => {
  const laneScore = scorePhrase("MIPI_DSI0_CLK_N")

  expect(laneScore).toBe(1.2)
  expect(scorePhrase("MIPI-CSI1-D2-P")).toBe(laneScore)
  expect(scorePhrase("MIPI_DSI2_CK_N")).toBe(laneScore)
  expect(scorePhrase("MIPI_DPHY_D3_N")).toBe(laneScore)
  expect(scorePhrase("MIPI_D0_P")).toBe(laneScore)
  expect(scorePhrase("MIPI_CSI_DP0")).toBe(laneScore)
  expect(scorePhrase("MIPI_DSI_DN1")).toBe(laneScore)
  expect(scorePhrase("pin14")).toBe(0.5)
  expect(scorePhrase("MIPI1")).toBe(0.5)
  expect(scorePhrase("MIPI_CTRL1")).toBe(0.5)
  expect(scorePhrase("CSI_CTRL1")).toBe(0.5)
  expect(scorePhrase("DSI_RESET1")).toBe(0.5)
  expect(scorePhrase("DPHY_STATUS1")).toBe(0.5)
  expect(scorePhrase("MIPI_CLK_EN1")).toBe(0.5)
  expect(scorePhrase("MIPI_D1")).toBe(0.5)
  expect(scorePhrase("MIPI_CSI_D0")).toBeLessThan(laneScore)
  expect(scorePhrase("MIPI_DSI_CLK")).toBeLessThan(laneScore)
  expect(scorePhrase("MIPI_DPHY_D3")).toBeLessThan(laneScore)
